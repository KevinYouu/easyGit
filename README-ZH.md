...
# 交互式切换分支(脏工作区可选携带修改 / 自动 stash)
easyGit branch-switch  # 或: easyGit sw

# 新建分支,可选推送并设置上游跟踪
easyGit branch-create  # 或: easyGit bc
...
| `branch-switch` | `sw` | 交互式切换分支(本地 + 远程)   | 分支间移动;远程自动建跟踪分支 |
| `branch-create` | `bc` | 新建分支,可选推送设上游跟踪   | 从任意基点开始新开发           |
...
| `clean`       | `cl` | 丢弃修改与删除未跟踪文件 | 可勾选的工作区清理 |
